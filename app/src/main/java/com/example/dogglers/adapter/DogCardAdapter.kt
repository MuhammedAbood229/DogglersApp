...
class DogCardAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DogCardViewHolder {
        ...
        val adapterLayout = when(layout){
            Layout.HORIZONTAL,Layout.VERTICAL -> LayoutInflater.from(parent.context).inflate(R.layout.vertical_horizontal_list_item,parent,false)
            Layout.GRID -> LayoutInflater.from(parent.context).inflate(R.layout.grid_list_item,parent,false)
            else -> "Something wrong"
        }
        return DogCardViewHolder(adapterLayout as View?)
    }
